...
## 9. 后续增强（三期，2026-08-27）

二期上线后，线上实测发现「三元组抽取畸形」「主动/自动重复」「正反偏好无法演化」三个更深层问题，遂进行三期重构：存储范式从三元组迁移到自然语言 fact，并新增语义去重与主动记忆调和。详见 [005c-memory-fact-semantic-dedup](./005c-memory-fact-semantic-dedup.md)。
